...
namespace Assignment10
{
    ...
    public class FlightNetwork
    {
        ...
        public void AddFlight(Flight flight)
        {
            ...
            {
                string originCity = airportCities.ContainsKey(destinationCode) ? airportCities[destinationCode] : "Unknown City";
        ...
        public void LoadFlightsFromCSV(string filename)
        {
            ...
            try
            {
                ...
                if(parts.Length < 5)
                {
                    Console.WriteLine($"Invalid data format on line {i + 1}: {lines}");
                }
        ...
        public Airport? GetAirport(string code)
        {
            ...
            return airports.Contains(upperCode) ? airports[upperCode] : null;
        }
        ...
        public List<string>? FindRoute(string origin, string destination)
        {
            ...
            string originUpper = origin.ToUpperInvariant();
            string destinatinoUpper = destination.ToUpperInvariant();

            if(!airports.ContainsKey(originUpper) || !airports.ContainsKey(destinatinoUpper))
            {
                return null;
            }
            if(originUpper == destinatinoUpper)
            {
            ...
            while(queue.Count > 0)
            {
                ...
                if (current.Equals(destinatinoUpper, StringComparison.OrdinalIgnoreCase))
                {
                    return ReconstructPath(parents, originUpper, destinatinoUpper);
                }
