...
namespace QueueLab
{
    ...
    class Program
    {
        ...
        private static DateTime sessionBegin = DateTime.Now;
        ...
        static void HandleSubmitTicket()
        {
            ...
            string ticketId = $"{ticketCounter:D3}";
        ...
        static void HandleProcessTicket()
        {
            ...
            {
                SupportedTicket ticket = ticketQueue.Dequeue();
                totalOperations++;
        ...
        static void HandleDisplayQueue()
        {
            ...
            Console.WriteLine("Current Support Queue (FIFO oder): ");
        ...
        static void HandleClearQueue()
        {
            ...
            if (ticketQueue > 0)
            {
                ...
                Console.WriteLine($"This will get rid of {ticketCount} tickets. Are you certain? (Y/N)");
                string? replyConfirmation = Console.ReadLine()?.ToLower() ?? "";

                if (replyConfirmation == "Y" || replyConfirmation = "yes")
                {
                    ticketQueue.Clear();
                    totalOperations++;
                ...
                {
                    Console.WriteLine("Clear operation cancelled. Nothing in queue.");
                }
            }
        }
        ...
        static void HandleUrgentTicket()
        {
            ...
            Console.WriteLine("Enter urgent issue description: ");
            ...
            Console.WriteLine($"Urgent ticket ID: {ticketId} submission successful!");
            Console.WriteLine($"Description: {description}");
            Console.WriteLine($"Position in queue{ticketQueue.Count}");
            Console.WriteLine($"Note: In a real system, this would jump to front of the queue.");
        }
        ...
        static void HandleSearchTicket()
        {
            if (ticketQueue > 0)
            {
                ...
                {
                    Console.WriteLine("Search term cannot be empty. Search terminated.");
                    return;
                }
                bool found = false;
                int position = 1;
                ...
                foreach (var ticket in ticketQueue)
                {
                    if (ticket.TicketId.ToLower().Contains(searchTerm.ToLower()) || ticket.Description.ToLower().Contains(searchTerm.ToLower()))
                    {
