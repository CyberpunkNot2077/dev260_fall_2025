...
namespace Lab8_Sets
{
    ...
    public class SetOperationsLab
    {
        ...
        public bool HasPermission(string userId, string permission)
        {
            ...
            //Defensive check: Does this user exist?
            if (userPermissions.ContainsKey(userId))
            {
                return false; //User not present, no permissions
            }
        ...
        public double CalculateRetentionRate()
        {
            ...
            if (enrolledLastQuarter == 0)
            {
                return 0.0;
            }

            var continuingStudents = FindContinuingStudents();
            double retentionRate = (double)continuingStudents.Count / (double)enrolledLastQuarter.Count * 100.0;
