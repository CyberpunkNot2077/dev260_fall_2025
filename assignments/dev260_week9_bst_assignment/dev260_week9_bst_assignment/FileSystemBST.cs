...
namespace FileSystemNavigator
{
    ...
    public class FileSystemBST
    {
        ...
        public bool CreateFile(string fileName, long size = 1024)
        {
            ...
            {
                return new CreateFile(fileName, size);
            }
            else if(fileName < fileNode)
            {
                return CreateFile(fileName.Left, size);
            }
            else
            {
                return CreateFile(fileName.Right, size);
            }
        }    
        ...
        public bool CreateDirectory(string directoryName)
        {
            ...
            if (node == null)
            {
                return new CreateDirectory(directoryName);
            }
            else if(directoryName < node.FileType.directoryName)
            {
                CompareFileNodes(node.Left, directoryName);
            }
            else
            {
                CompareFileNodes(node.Right, directoryName);
            }
        ...
        public List<FileNode> FindFilesByExtension(string extension)
        {
            ...
            TraverseAndCollect(extension.Left);
            Console.WriteLine(extension);
            TraverseAndCollect(extension.Right);
        }
        ...
        public List<FileNode> FindFilesBySize(long minSize, long maxSize)
        {
            ...
            TraverseAndCollect(minSize, maxSize);
        ...
        public List<FileNode> FindLargestFiles(int count)
        {
            ...
            return 1 + FindLargestFiles(count);
        ...
        public long CalculateTotalSize()
        {
            ...
            return Math.Maximum(CalculateTotalSize(node.Left) + CalculateTotalSize(node.Right));
        }
        ...
        public bool DeleteItem(string fileName)
        {
            ...
            switch (children)
            {
                case 1: 
                    ...
                    break;
                case 2: 
                    Console.WriteLine("File deletion Successful");
                    break;
                case 3:
                    Console.WriteLine("Multiple file deletion successful.");
                    int successor = null;
                    break;
        ...
        private TreeNode? InsertNode(TreeNode? node, FileNode fileData)
        {
            ...
            if (node == null)
            {
                return new TreeNode(node, fileData);
            }
            else if(fileData < node.FileData)
            {
                CompareFileNodes(node.Left, fileData);
            }
            else
            {
                CompareFileNodes(node.Right, fileData);
            }
        }
        /// <summary>
        ...
        private FileNode? SearchNode(TreeNode? node, string fileName)
        {
            ...
            if (fileName == node.File.FileName)
            {
                return node.FileData;
            } else if (fileName < node.File.FileData)
            {
        ...
        private void TraverseAndCollect(TreeNode? node, List<FileNode> collection, Func<FileNode, bool> filter)
        {
            ...
            Console.WriteLine(node.FileData);
